...
async def cmd_start(update, ctx):
    ...
    await update.message.reply_text(
        ...
        "Добро пожаловать в <b>НИКЕР</b> — маркет Telegram-ников.\n\n"
        "⛏️ Бот майнит уникальные ники и проверяет доступность\n"
...
async def cb_about(update, ctx):
    ...
    await q.answer()
    text = (
        "ℹ️ <b>Как работает НИКЕР</b>\n\n"
        "🤖 <b>Автоматическая генерация</b>\n"
        "Бот придумывает ники и проверяет доступность — на Telegram и Fragment. "
...
async def successful_payment(update, ctx):
    ...
    await update.message.reply_text(
        f"🧾 <b>ЧЕК ОБ ОПЛАТЕ</b>\n"
        f"{'─' * 28}\n"
        f"Ник:    <tg-spoiler>@{username}</tg-spoiler>\n"
        f"Сумма:  <b>{stars} ⭐</b>\n"
        f"Статус: ✅ Оплачено\n"
        f"{'─' * 28}\n"
        f"👆 Нажми на спойлер чтобы открыть ник\n"
